...
function Tools() {
  ...
  const extractVideo = async (event) => {
    ...
    if (!validatePlatformUrl(cleanUrl, activeTool.slug)) {
      setError(`Please enter a valid public ${activeTool.category} URL.`);
      return;
    }
    ...
    try {
      ...
      const availableHeights = available
        .map((item) => Number(item.height))
        .filter(Boolean);

      const preferred = [1080, 720, 480, 360].find((q) =>
        availableHeights.includes(q)
      );
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          "Unable to extract media. Please ensure the link is public."
      );
      setProgress(0);
    } finally {
      setIsExtracting(false);
  ...
  const downloadVideo = async () => {
    ...
    try {
      ...
      const filename = `${activeTool.category.toLowerCase()}-${selectedQuality || "media"}-${Date.now()}.${ext}`;
      ...
      let message = "Download failed. The media source may be private or restricted.";
    ...
    <div className="min-h-screen bg-[#090d16] font-sans text-slate-100 selection:bg-indigo-500/30 selection:text-white relative overflow-x-hidden">
      ...
      <section className="relative pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center">
        ...
        <p className="mt-6 max-w-2xl mx-auto text-base sm:text-lg text-slate-400 leading-relaxed">
          Extract and download public media from YouTube, Facebook, Instagram, and Pinterest.
        </p>
          ...
          <div className="relative w-full max-w-2xl rounded-3xl bg-slate-900 border border-slate-800 p-6 sm:p-8 shadow-2xl my-8">
            ...
            {error && (
              <div className="mt-5 rounded-2xl border border-rose-500/30 bg-rose-500/10 p-4 text-sm text-rose-300">
                <strong className="block mb-1 font-bold">Processing Failed</strong>
              ...
              <div className="py-8 text-center">
                ...
                <p className="mt-2 text-sm text-slate-400">
                  {downloadResult.isPhoto
                    ? "High-Resolution Image has been saved."
                    : `${downloadResult.quality}p Video has been downloaded.`}
                </p>
                ...
                <a
                  ...
                  className="mt-6 inline-flex w-full items-center justify-center rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 px-6 py-4 text-sm font-bold text-white shadow-lg shadow-emerald-500/20"
                >
                  ⬇ Save Again
                </a>
                ...
                <button
                  ...
                  className="mt-3 w-full rounded-2xl border border-slate-700 bg-slate-800/50 py-3.5 text-sm font-semibold text-slate-300 hover:bg-slate-800"
                >
              ...
              <div className="mt-6">
                  ...
                  <div className="flex gap-4 items-center">
                    {videoInfo.thumbnail && (
                      <img
                        ...
                        alt="Thumbnail"
                        className="w-28 h-20 sm:w-32 sm:h-20 object-cover rounded-xl border border-slate-800 flex-shrink-0"
                      />
                    )}
                    <div className="min-w-0">
                      <h4 className="font-bold text-white line-clamp-2 text-sm sm:text-base">
                ...
                <div className="mt-7 flex gap-3">
                  <button
                    ...
                    disabled={isDownloading}
                    onClick={processAnother}
                    className="flex-1 rounded-xl border border-slate-700 bg-slate-800 py-3.5 text-sm font-semibold text-slate-300 hover:bg-slate-700 transition disabled:opacity-50"
                  >
                    Change Link
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 py-3.5 text-sm font-bold text-white shadow-lg shadow-indigo-500/25 disabled:opacity-50 hover:brightness-110 transition"
                  >
              ...
              <form onSubmit={extractVideo} className="mt-6 space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-slate-800 bg-[#0c1322] px-4 py-3.5 text-sm text-white placeholder-slate-600 outline-none focus:border-indigo-500 transition"
                  />
                </div>
                ...
                {isExtracting && (
                  <div>
                    <div className="flex justify-between text-xs mb-2">
                      <span className="text-slate-400">Fetching media stream...</span>
                      <span className="text-indigo-400 font-bold">{progress}%</span>
                    </div>
                    <div className="h-2.5 rounded-full bg-slate-800 overflow-hidden">
                      <div
                ...
                <div className="flex justify-end gap-3 pt-3 border-t border-slate-800">
                  <button
                    ...
                    disabled={isExtracting}
                    onClick={closeModal}
                    className="rounded-xl px-5 py-3 text-sm font-semibold text-slate-400 hover:bg-slate-800 transition"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={isExtracting}
                    className="rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-500/25 disabled:opacity-50 hover:brightness-110 transition"
                  >
